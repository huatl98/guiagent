...
import log_recorder
import experience_pool
import log_replay
import UserManager
from UserManager import UserManager
from UserManager import ocr_process
...
class MiniCPMWrapper(LlmWrapper, MultimodalLlmWrapper):
    ...
    def predict_mm(
        ...
        try:
            # 使用临时文件上下文管理器
            temp_img_path = f"temp_{int(time.time())}.png"
            try:
                Image.fromarray(images[0]).save(temp_img_path)
                ocr_results = ocr_process(temp_img_path)
            finally:
                if os.path.exists(temp_img_path):
                    os.remove(temp_img_path)
        except Exception as e:
            logging.warning(f"用户交互记录失败（不影响主功能）: {str(e)}")
        if text_prompt != self._last_recorded_input:
            try:
                user_manager._record_user_interaction(
                    user_input=text_prompt,
                    ocr_results=ocr_results
                )
                self._last_recorded_input = text_prompt
            except Exception as e:
                logging.warning(f"用户交互记录失败: {str(e)}")
        user_dir = user_manager.get_user_dir(user_manager.current_user_id)
        with log_recorder.TaskLogger(log_dir=os.path.join(user_dir, "task_logs")) as task_logger: 
